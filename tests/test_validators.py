...
class TestValidators(unittest.TestCase):
    ...
    def test_validate_no(self):
        ...
        request = {'request': {'command': 'нет', 'nlu': {'intents': {'YANDEX.REJECT': {}}}}}
        validator = IntentValidator(request)
    ...
    def test_validate_draw(self):
        ...
        request = {'request': {'command': 'ничья', 'nlu': {'intents': {'DRAW': {}}}}}
        validator = IntentValidator(request)
        self.assertTrue(validator.validate_draw())

    def test_validate_resign(self):
        """Тест валидации сдачи."""
        request = {'request': {'command': 'сдаюсь', 'nlu': {'intents': {'RESIGN': {}}}}}
        validator = IntentValidator(request)
    ...
    def test_validate_undo(self):
        ...
        request = {'request': {'command': 'отменить ход', 'nlu': {'intents': {'UNDO': {}}}}}
        validator = IntentValidator(request)
        self.assertTrue(validator.validate_undo())

    def test_validate_repeat_last_move(self):
        """Тест валидации повтора последнего хода."""
        request = {'request': {'command': 'повторить ход', 'nlu': {'intents': {'REPEAT_LAST_MOVE': {}}}}}
        validator = IntentValidator(request)
